...
##### 6. Fit two elastic net models: one for $Z_1$ and one for $Z_2$. Use `alpha = 0.5`. Select an appropriate number of non-zero loadings through a sensible choice of $\lambda$.
...
# For PC1
set.seed(45)
fit_loadings1 <- cv.glmnet(X, Z[, 1], alpha = 0.5)
plot(fit_loadings1, main = "PC1")

# For PC2
set.seed(45)
fit_loadings2 <- cv.glmnet(X, Z[, 2], alpha = 0.5)
