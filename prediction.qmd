...
\[
  \text{var}\left[\hat{\mathbf\beta}\right] = (\mathbf{X^TX}+\lambda \mathbf{I})^{-1} \mathbf{X}^T\mathbf{X} (\mathbf{X^TX}+\lambda \mathbf{I})^{-1}\sigma^2 = \mathbf{V}\boldsymbol{\Delta}^2(\boldsymbol{\Delta}^2+\lambda\mathbf{I})^{-2}\mathbf{V}^T\sigma^2.
\]
...
\]
with $L_0$ norm $\Vert \boldsymbol{\beta} \Vert_0 = \sum_{j=1}^p \beta_j^0 = p$.
...
roc <-
  ...
  ggplot(aes(d = known.truth, m = pi, color = model)) +
  geom_roc(n.cuts = 0) +
  xlab("1-specificity (FPR)") +
  ylab("sensitivity (TPR)")
...
roc <- rbind(
  dfLassoOpt |> mutate(model = "lasso"),
  dfElast |> mutate(model = "elast. net")) |>
  ggplot(aes(d = known.truth, m = pi, color = model)) +
  geom_roc(n.cuts = 0) +
  xlab("1-specificity (FPR)") +
  ylab("sensitivity (TPR)")
