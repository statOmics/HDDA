...
$\rightarrow$ Probability to have a false positive (FP) among all m simultaneous
test $>>>  \alpha= 0.05$
...
### Method of Šidák: invert FWER to significant level for individual test
...
\]
so that the simultaneous testing procedure controls the FWER at the desired level (method of Šidák).
...
- When the assumptions hold, it guarantees
  \[
    \text{FDR}=\text{E}\left[FP/R\right]=\text{E}\left[\text{FDP}\right] \leq \frac{m_0}{m} \alpha \leq \alpha .
  \]
...
- It can be shown  that the BH-FDR method weakly controls the FWER, i.e. it controls the FWER if all features are null ($m_0=m$).
...
- Efron argues that $\text{E}_{f_1}\left[\text{fdr}(Z)<\alpha\right]$ is a kind of extension of the definition of the power of a test: it provides the expected fraction of non-null features with a local fdr below $\alpha$.

- The graph shows, for examples, that with $\alpha=0.20$ we only have $\text{E}_{f_1}\left[\text{fdr}(Z)<\alpha\right] =0.24$, i.e. only $24\%$ of the non-nulls are expected to be discovered.

- At the bottom of the graph we read Efdr$=0.486$. Which indicates that the local fdr for a typical non-null feature is expected to be 48.6% which is rather large. The study is not well powered!
