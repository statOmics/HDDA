...
$$
\|\mathbf Y-\mathbf{X}\boldsymbol \beta\|^2_2 +  \alpha \lambda\|\boldsymbol \beta\|_1 + \frac{(1 - \alpha)}{2} \lambda\|\boldsymbol \beta\|^2_2
$$
...
plot_sparse_PCA(
  ...
  as.vector(coef(fit_loadings2, s = 5))
)
```
...
In separating normal and tumour tissues, SPCA performs virtually the same as PCA. By increasing $\lambda$, more sparsity is induced in the loadings, while the overall visual impression remains comparable. 
