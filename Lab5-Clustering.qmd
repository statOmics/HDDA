...
ggplot(wine_prop_var, aes(PC, var)) +
  geom_point() +
  geom_line() +
  geom_vline(xintercept = 7.25, col = "firebrick") +
  scale_x_continuous(breaks = 1:ncol(wine_pca$x)) +
  labs(y = "Proportion of variance")
```

The vertical line marks where the cumulative variance reaches 90%, which falls between the 7th and 8th PC. The first 7 PCs explain about 89% of the variance in the `wine` data, so we retain 7 PCs.
